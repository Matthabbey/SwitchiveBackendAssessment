...
export class PaginationInterceptor implements NestInterceptor {
  constructor(private readonly paginationService: PaginationService, readonly entity: string) {}
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    const page_size = this.paginationService.getPageSize(Number(query.page_size));
      ...
      return next.handle().pipe(
        map((data) => {
          ...
          const transformedData = {
            message: tranformData ? tranformData.message || 'Successful' : 'Successful',
            statusCode: response.statusCode,
            [this.entity]: tranformData ? tranformData.data || tranformData : data,
          };
